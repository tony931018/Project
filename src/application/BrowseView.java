...
public class BrowseView {
    public static void show(Stage stage) {
        ...
        VBox content = new VBox(15, title, search, category, cards);
    ...
    private static VBox card(Stage stage, Equipment item) {
        ...
        Button report = new Button("Report Issue");
        report.setOnAction(e -> {
            DataStore.reports.add(
                    "User: " + DataStore.currentUser
                            + " | Listing: " + item.getName()
                            + " | Issue: Reported by user | Status: Pending"
            );

            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setHeaderText(null);
            alert.setContentText("Report sent to admin.");
            alert.showAndWait();
        });

        VBox box = new VBox(7, name, info, price, details, report);
